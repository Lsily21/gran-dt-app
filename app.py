...
def procesar_fixture(url):
    ...
    try:
        ...
        return pd.DataFrame(columns=['Equipo', 'Rival'])
    ...
    for _, row in df.iterrows():
        ...
        if len(vals) >= 2:
            eq_local = vals[0]
            eq_visita = vals[1]
            mapping_list.append({'Equipo': eq_local, 'Rival': eq_visita})
            mapping_list.append({'Equipo': eq_visita, 'Rival': eq_local})
...
if st.button("🚀 Procesar Datos y Actualizar Scouting"):
    ...
    df_fix['Rival_key'] = df_fix['Rival'].apply(normalizar_nombre)
    df_full['Equipo_key'] = df_full['Equipo'].apply(normalizar_nombre)
    
    df_full = df_full.merge(df_fix[['Equipo_key', 'Rival']], on='Equipo_key', how='left')
    df_full['Rival'] = df_full['Rival'].fillna('Libre / S/D')
...
if 'df_full' in st.session_state:
    ...
    tabs = st.tabs([
        f"👕 Armado Táctico (Plantel Ideal 15 - F{NUM_FECHA})", 
        "🧤 ARQ", 
        "🛡️ DEF", 
        "👟 VOL", 
        "⚽ DEL",
